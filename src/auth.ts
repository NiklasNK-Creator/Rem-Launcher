...
function offlineUuid(name: string): string {
  ...
  // version 3 (md5) + RFC 4122 variant
  const v = `${h.slice(0, 8)}-${h.slice(8, 12)}-3${h.slice(13, 16)}-${((parseInt(h.slice(16, 18), 16) & 0x3f) | 0x80).toString(16)}${h.slice(18, 20)}-${h.slice(20)}`;
  return v.replaceAll("-", "");
}
