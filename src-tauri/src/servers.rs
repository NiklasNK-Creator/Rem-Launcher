...
pub fn add_server(app: AppHandle, name: String, address: String) -> Result<Vec<ServerEntry>, String> {
    let address=address.trim().to_string(); if !valid_address(&address){return Err("invalid server address".into());}
    let mut v=load(&app); let id=format!("server:{}", address.to_lowercase()); v.retain(|s|s.id!=id); v.push(ServerEntry{id,name:name.trim().to_string(),address,last_ping_ms:None}); save(&app,&v)?; Ok(v)
}
#[tauri::command]
